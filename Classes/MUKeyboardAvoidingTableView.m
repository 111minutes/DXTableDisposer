...
- (CGFloat)idealOffsetForView:(UIView *)view withSpace:(CGFloat)space
{
    ...
    if (_selectIndexInputField != NSNotFound) {
        [_keyboardToolbar selectedInputFieldIndex:_selectIndexInputField allCountInputFields:[_objectsInKeyboard count]];
    }
...
- (void)addObjectsForKeyboard:(NSArray *)objectsForKeyboard
{
    for (id obj in objectsForKeyboard) {
        if ([obj conformsToProtocol:@protocol(UITextInputTraits)]) {
            if ([obj respondsToSelector:@selector(inputAccessoryView)] && _keyboardToolbar) {
                [obj setInputAccessoryView:_keyboardToolbar];
            }
...
- (void)createToolbar
{
    _keyboardToolbar = [[MUKeyboardToolbar alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, 44.f)];
    _keyboardToolbar.delegate = self;
    [self setInputAccessoryView:_keyboardToolbar];
}
